...
public class Multipower_RSA {
	...
	public Multipower_RSA(BigInteger e, BigInteger d, BigInteger N)
	{
		this.e=e;
		this.d=d;
		this.N=N;
	...
	public static BigInteger decrypt(BigInteger msg)
	{
		...
		//System.out.println("The negaive is: "+ n1);
		BigInteger c1=msg.mod(p.multiply(p));
		BigInteger m1=c1.modPow(d1.subtract(ONE), p);
		BigInteger k=m1.multiply(c1.mod(p));
		BigInteger A=(k.modPow(e, p.multiply(p))).multiply(n1);
		BigInteger A1=A.add(msg.mod(p.multiply(p)));
		BigInteger m12=m1.multiply(A1.mod(p.multiply(p)));
		BigInteger m13=m12.multiply((e.modPow(n1, p)).mod(p.multiply(p)));
		BigInteger m14=m13.add(k.mod(p.multiply(p)));
		
		//decryption
		BigInteger c2=msg.mod(q);
		BigInteger m2=c2.modPow(d2, q);
		BigInteger v1=m2.subtract(m14.mod(q));
		BigInteger v12=v1.multiply(((p.multiply(p)).modPow(n1, q)).mod(q));
		BigInteger M1=v12.multiply((p.multiply(p).mod(N)));
		BigInteger M12=M1.add(m14.mod(N));
		
		BigInteger d1=M12.multiply(q).multiply(q.modInverse(p));
